...
static void InitMorphQueue ( void )
{
	...
	morph.inc      = 1;		// Morph progression increment (step size per frame).
	morph.flag     = 0;		// No morph in progress.
	morph.timer    = 0;		// Reset the wait timer.
	morph.count    = 0;		// Start at the first morph in the queue.
	morph.complete = 0;		// Mark morph as complete (ready to advance).
	...
	while ( program_flags.quit == 0 )
  	{
		...
		WaitRetrace    ();					// Synchronize with the vertical retrace to avoid tearing.
		...
		ClearScreen ( 0, vga_buffer );		// Clear the back buffer for the next frame.
